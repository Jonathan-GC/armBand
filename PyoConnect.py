...
'''Importacion de las librerias pymouse and keyboard
***************************************************'''
try:
...
class Myo(MyoRaw):
    ...
    def emg_handler(self, emg, moving):
        #funicon de Variables que guardan los datos de electormiografia
        while (emg[2]>900): #si el electrodo 2
            Salto=1
            #cnt.posture(0) #Cambio de postura para el Jumping con 
            print("cambio")# el cambio de valor leido por el electrodo
	    #cnt.posture(1)
            if(emg[2]<900):
                print("joder tio")
	    #cnt.posture(1)   #Vuelve a su postura natural
    ...
    def imu_handler(self, quat, acc, gyro):
        ...
        Vel=(((self.current_pitch)*180/math.pi)*2)-75 #ecuacion que define la vel del Jumpong
        Giro=(((self.current_roll)*180/math.pi))*0.6667 - 28.3333 #ecuacion que define la velocidad de giro
